#include "LoLCha.h"
LoL::LoL(int N1, int N2, float a, float b, float x0, float dx, int m,string filename)
    : N1(N1), N2(N2), a(a), b(b), x0(x0), dx(dx), m(m), filename(filename) {
    ...
    aa = f1();
    dd.write((char*)aa, sizeof(int) * (N2 - N1 + 1));
    bb = f2();
    dd.write((char*)bb, sizeof(float) * m);
    dd.close();
}
